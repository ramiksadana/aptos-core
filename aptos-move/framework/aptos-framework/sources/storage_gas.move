module aptos_framework::storage_gas {
    ...
    // Error if storage_gas_config does not exist.
    const ESTORAGE_GAS_CONFIG: u64 = 0;
    // Error if storage_gas does not exist.
    const ESTORAGE_GAS: u64 = 1;
    // Error if max_gas <= min_gas.
    const EINVALID_GAS_RANGE: u64 = 2;
    // Error if target_usage == 0.
    const EZERO_TARGET_USAGE: u64 = 3;
    // Error if target_usage is too big that may cause other arithmetic in this module overflow.
    const ETARGET_USAGE_TOO_BIG: u64 = 4;
    // A valid curve points must be monotonically increasing on x axis and non-decreasing on y axis.
