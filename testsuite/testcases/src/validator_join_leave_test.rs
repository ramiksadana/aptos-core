...
impl NetworkTest for ValidatorJoinLeaveTest {
    fn run<'t>(&self, ctx: &mut NetworkContext<'t>) -> Result<()> {
        ...
        for validator_index in 0..num_validators / 3 {
            runtime.block_on(async {
                cli.leave_validator_set(validator_cli_indices[validator_index], None)
                    .await
                    .unwrap();
        ...
        for validator_index in 0..num_validators / 3 {
            runtime.block_on(async {
                cli.join_validator_set(validator_cli_indices[validator_index], None)
                    .await
                    .unwrap();
